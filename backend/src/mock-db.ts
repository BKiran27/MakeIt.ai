export class MockDb {
  ...
  private projectMaterials: any[] = [];
  private tools: any[] = [];
  private projectTools: any[] = [];
  private likes: any[] = [];
  private comments: any[] = [];
  private savedProjects: any[] = [];
  private subscriptions: any[] = [];

  constructor() {
    // Populate default admin/user mock account
    this.users.push({
      id: 'mock-user-id',
    ...
    return {
      create: async (args: any) => {
        ...
        const newProject = {
          ...
          createdAt: new Date(),
          updatedAt: new Date(),
          steps: stepsList.map((s: any, idx: number) => ({
            id: Math.random().toString(36).substring(7),
  ...
  async $queryRawUnsafe(query: string, ...params: any[]) {
    // Cosine distance vector search fallback returns top matching projects
